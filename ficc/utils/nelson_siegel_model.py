...
def iterate_backward_by_minute(dictionary: dict, most_recent_datetime: datetime) -> datetime:
    ...
    def iterate_backward(date_and_time, num_mins_back, max_num_times=-1):
        ...
        while date_and_time.strftime(YEAR_MONTH_DAY_HOUR_MIN) in dictionary:    # find the `date_and_time` that is at most `num_mins_back` minutes in the past
    ...
    def iterate_forward(date_and_time, num_mins_forward):
        ...
        while date_and_time.strftime(YEAR_MONTH_DAY_HOUR_MIN) in dictionary:    # after finding `date_and_time` to most `num_mins_back` minutes in the past, increment up by 1 minute until we cannot find it anymore
...
def load_model_parameters(target_datetime: datetime, nelson_params: dict, scalar_params: dict, shape_parameter: dict, end_of_day: bool):
    ...
    nelson_coeff = nelson_params[target_datetime_for_nelson_params].values()
...
def yield_curve_level(maturity: float, target_datetime, nelson_params, scalar_params, shape_params, end_of_day: bool):
    ...
    const, exponential, laguerre = nelson_siegel_coef
    exponential_mean, exponential_std, laguerre_mean, laguerre_std = scaler_daily_parameters
    return predict_yield_curve_level(maturity, const, exponential, laguerre, exponential_mean, exponential_std, laguerre_mean, laguerre_std, shape_parameter)
